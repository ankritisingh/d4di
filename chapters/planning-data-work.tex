...
On Windows, this will be something like ``C:/`` or ``This PC''. (It is never your desktop.)
...
\texttt{iefolder} sets up your project folder according to DIME Analytics' folder 
template, although we understand that this template does not fit all projects. There
are even projects at DIME that are too different from a typical project to 
use this template, but the point is that it is important that your team carefully
plans your folder setup, and DIME Analytics' folder
template is always a good starting point.
...
What may differ is the path to the project root folder (the highest-level shared folder).
...
By follow, we mean someone external to the project who has the master script 
and all the input data can
...
to be created and stored separately in GitHub\sidenote{\textbf{GitHub:} the most commonly used web hosting of projects using \textbf{Git}.}.
...
\LaTeX.\sidenote{\url{https://www.latex-project.org} and \url{https://github.com/worldbank/DIME-LaTeX-Templates}}
...
This book, for example, was written in \LaTeX, and version 
controlled using Git and GitHub\sidenote{\url{https://github.com/worldbank/d4di}}.
